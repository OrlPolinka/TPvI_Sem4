...
namespace _5test_aspa005_3
{
    public class Ttest
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            await test.ExecuteDELETE<int?>($"http://localhost:{port}/A/-1--25", (int? x, int? y, int status) => (x == null && y == null && status == 404) ? Test.OK : Test.NOK);
            await test.ExecuteDELETE<int?>($"http://localhost:{port}/A/25-101", (int? x, int? y, int status) => (x == null && y == null && status == 404) ? Test.OK : Test.NOK);
            Console.WriteLine("B ------------------------------");
            await test.ExecuteGet<float?>($"http://localhost:{port}/B/2.5", (float? x, float? y, int status) => (x == 2.5f && y == null && status == 200) ? Test.OK : Test.NOK); 
            ...
            await test.ExecuteDELETE<string?>($"http://localhost:{port}/B/2.5-3.2", (string? x, string? y, int status) => (x == "2.5" && y == "3.2f" && status == 200) ? Test.OK : Test.NOK);
            Console.WriteLine("C ---------------------------------");
            ...
            await test.ExecutePOST<bool?>($"http://localhost:{port}/C/true,false", (bool? x, bool? y, int status) => (x == true && y == false && status == 200) ? Test.OK : Test.NOK);
            ...
            await test.ExecutePOST<DateTime?>($"http://localhost:{port}/D/2025-02-25|2025-03-25", (DateTime? x, DateTime? y, int status) => (x == new DateTime(2025, 02, 25) && y == new DateTime(2025, 03, 25) && status == 200) ? Test.OK : Test.NOK); 
            ...
            Console.WriteLine("F ------------------------------------");
            await test.ExecuteGet<string?>($"http://localhost:{port}/F/smw@belstu.by", (string? x, string? y, int status) => (x == "smwbelstu.by" && y == null && status == 200) ? Test.OK : Test.NOK);
